...
import { salesReportRequest } from '../../apiRequest/reportApiRequest'

const SalesReport = () => {
    let fromDateRef, toDateRef = useRef()

    let data = useSelector((state)=>state.report.SalesByDateRange)
    ...
    const onCreateReport = async () =>{
        if(IsEmpty(fromDateRef.value)){
            ...
            await salesReportRequest(fromDateRef.value, toDateRef.value)
        }
    }
    ...
    const onExport = (exportType,data) => {
        if(data.length>0){
            ...
            exportFromJSON({data: ReportData, fileName: 'SalesReport from '+fromDateRef.value+' to '+toDateRef.value, exportType: exportType })
        }
    }
    return (
        <Fragment>
