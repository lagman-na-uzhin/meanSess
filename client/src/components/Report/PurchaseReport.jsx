...
import { purchaseReportRequest } from '../../apiRequest/reportApiRequest'
...
const PurchaseReport = () => {
    ...
    let data = useSelector((state)=>state.report.PurchaseByDateRange)
    ...
    const onCreateReport = async () =>{
        if(IsEmpty(fromDateRef.value)){
            ...
            await purchaseReportRequest(fromDateRef.value, toDateRef.value)
        }
    }
    ...
    const onExport = (exportType,data) => {
        if(data.length>0){
            ...
            exportFromJSON({data: ReportData, fileName: 'PurchaseReport from '+fromDateRef.value+' to '+toDateRef.value, exportType: exportType })
        }
    }
    return (
        <Fragment>
