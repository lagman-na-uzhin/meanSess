...
import { expensesReportRequest } from '../../apiRequest/reportApiRequest'
...
const ExpenseReport = () => {
    ...
    let data = useSelector((state)=>state.report.ExpensesByDateRange)
    ...
    const onCreateReport = async () =>{
        if(IsEmpty(fromDateRef.value)){
            ...
            await expensesReportRequest(fromDateRef.value, toDateRef.value)
        }
    }
    ...
    const onExport = (exportType,data) => {
        if(data.length>0){
            ...
            data.map((item,i)=>{
                let listItem={
                    "#No" : i+1,
                    "Expense Type":item['Type'][0]['title'],
                    "Amount":item['total'],
                    "Note":item['note'],
            ...
            exportFromJSON({data: ReportData, fileName: 'ExpenseReport', exportType: exportType })
        }
    }
    return (
        <Fragment>
