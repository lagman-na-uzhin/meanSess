...
import { deletePurchaseRequest, purchaseListRequest, purchaseProductsRequest } from '../../apiRequest/purchaseApiRequest';
...
const PurchaseList = () => {
    ...
    let Purchases = useSelector((state) => (state.purchase.Purchase));
    let Total = useSelector((state) => (state.purchase.Total))
    let PurchaseItems = useSelector((state)=> (state.purchase.purchaseItem))
    ...
    const onShow = async (id) =>{
        await purchaseProductsRequest(id)
    }
                                                    ...
                                                    <tbody>
                                                        {
                                                            Purchases.map((item, i) =>
                                                                <tr key={i}>
                                                                    ...
                                                                    <td><p className="text-xs text-start">
                                                                        <CurrencyFormat value={item.shippingCost ? item.shippingCost : 0} displayType={'text'} thousandSeparator={true} prefix={'$'} />
                                                                    </p></td>
                                                                    <td><p className="text-xs text-start">
